...
var BubbleShapes=(function(){
  ...
  function createSVG(b){
    ...
    if(shape==='oval') drawOval(svg,fill,stroke,strokeW,dashArr);
    else if(shape==='rectangle') drawRect(svg,fill,stroke,strokeW,dashArr,false);
    else if(shape==='parallelogram') drawRect(svg,fill,stroke,strokeW,dashArr,true);
    else if(shape==='cloud') drawCloud(svg,fill,stroke,strokeW,dashArr);
    else if(shape==='spike') drawSpike(svg,fill,stroke,strokeW,dashArr);
    if(borderStyle==='double'){
      if(shape==='oval'){
        ...
        inner.setAttribute('stroke-width','2');svg.appendChild(inner);
      }
    }
    const tail=b.tail||b.tailDir||'none';
    if(tail!=='none'&&borderStyle!=='none'){
      if(b.type==='thought'||shape==='cloud') drawThoughtTail(svg,tail,fill,stroke);
      else drawTail(svg,tail,fill,stroke);
    } else if(tail!=='none'&&borderStyle==='none'){
      if(b.type==='thought'||shape==='cloud') drawThoughtTail(svg,tail,fill,'none');
      else drawTail(svg,tail,fill,'none');
    }
    return svg;
  }
  ...
  function drawOval(svg,fill,stroke,strokeW,dashArr){
    ...
    ell.setAttribute('stroke-width',strokeW);
  ...
  function drawCloud(svg,fill,stroke,strokeW,dashArr){
    ...
    const d='M25,80 C8,80 2,65 8,52 C2,42 8,25 22,22 C25,8 42,2 55,10 C62,2 80,2 85,15 C98,18 100,35 92,46 C100,58 95,78 80,80 Z';
  ...
  function drawSpike(svg,fill,stroke,strokeW,dashArr){
    const pts=[];const cx=50,cy=46,rx=44,ry=38,spikes=14;
    for(let i=0;i<spikes;i++){
      ...
      pts.push((cx+Math.cos(a2)*rx*1.25).toFixed(1)+','+(cy+Math.sin(a2)*ry*1.25).toFixed(1));
    }
    const poly=document.createElementNS(NS,'polygon');
    poly.setAttribute('points',pts.join(' '));poly.setAttribute('fill',fill);
  ...
  function drawTail(svg,tail,fill,stroke){
    const dir=TAIL_DIRS[tail];if(!dir)return;
    const poly=document.createElementNS(NS,'polygon');
    poly.setAttribute('points',dir.ax+','+dir.ay+' '+dir.px+','+dir.py+' '+dir.bx+','+dir.by);
    poly.setAttribute('fill',fill);
    if(stroke!=='none'){poly.setAttribute('stroke',stroke);poly.setAttribute('stroke-width','3');poly.setAttribute('stroke-linejoin','round');}
    svg.appendChild(poly);
  ...
  function drawThoughtTail(svg,tail,fill,stroke){
    ...
    dots.forEach(function(d,i){
      ...
      if(stroke!=='none'){c.setAttribute('stroke',stroke);c.setAttribute('stroke-width','2.5');c.setAttribute('stroke-dasharray','4,2');}
      svg.appendChild(c);
    });
  }
  ...
  function drawOnCanvas(ctx,b,ox,oy){
    ...
    var tail=b.tail||b.tailDir||'none';
    ctx.save();
    var cx=x+w/2,cy=y+h*0.46,rx=w*0.47,ry=h*0.40;
    if(borderStyle==='dashed')ctx.setLineDash([6,4]);
    else if(borderStyle==='dotted')ctx.setLineDash([2,3]);
    else ctx.setLineDash([]);
    if(tail!=='none'){
      if(b.type==='thought'||shape==='cloud'){
        var dots=getThoughtDotsCanvas(tail,x,y,w,h);
        dots.forEach(function(d){
          ctx.beginPath();ctx.arc(d[0],d[1],d[2],0,Math.PI*2);
          ctx.fillStyle=fill;ctx.fill();
          if(borderStyle!=='none'){ctx.setLineDash([3,2]);ctx.strokeStyle=stroke;ctx.lineWidth=2;ctx.stroke();}
          ctx.setLineDash([]);
        });
      }else{
        var tp=getTailPolyCanvas(tail,x,y,w,h);
        if(tp){
    ...
    if(shape==='oval'){
      ...
      ctx.beginPath();
      if(shape==='parallelogram'){var sk=w*0.08;ctx.moveTo(x+sk+w*0.03,y+h*0.03);ctx.lineTo(x+w*0.97,y+h*0.03);ctx.lineTo(x+w*0.97-sk,y+h*0.89);ctx.lineTo(x+w*0.03,y+h*0.89);}
      else{roundRect(ctx,x+w*0.03,y+h*0.03,w*0.94,h*0.86,w*0.04);}
      ctx.closePath();ctx.fillStyle=fill;ctx.fill();
      if(borderStyle!=='none'){ctx.strokeStyle=stroke;ctx.lineWidth=2.5;ctx.stroke();}
    }else if(shape==='cloud'){
    ...
    var lh=fs*1.3;var totalTH=lines.length*lh;
    lines.forEach(function(line,i){ctx.fillText(line,cx,cy-totalTH/2+lh*0.5+i*lh);});
    ctx.restore();
  }
  ...
  function drawCloudCanvas(ctx,x,y,w,h,fill,stroke,borderStyle){
    ctx.beginPath();var cx0=x+w*0.5,cy0=y+h*0.46;var bumps=12,rr=Math.min(w,h)*0.42;
    for(var i=0;i<=bumps;i++){var a=(i/bumps)*Math.PI*2;var br=rr*(0.85+0.15*Math.cos(a*3));
      var px=cx0+Math.cos(a)*br*(w/h>1?1.1:0.9);var py=cy0+Math.sin(a)*br*(h/w>1?1.1:0.9);
      if(i===0)ctx.moveTo(px,py);
      else{var ca=a-(1/bumps)*Math.PI*2;var cpx=cx0+Math.cos(ca+0.5/bumps*Math.PI*2)*(br*1.15)*(w/h>1?1.1:0.9);
        var cpy=cy0+Math.sin(ca+0.5/bumps*Math.PI*2)*(br*1.15)*(h/w>1?1.1:0.9);ctx.quadraticCurveTo(cpx,cpy,px,py);}
    }
    ctx.closePath();ctx.fillStyle=fill;ctx.fill();
    if(borderStyle!=='none'){ctx.strokeStyle=stroke;ctx.lineWidth=2.5;ctx.stroke();}
  }
  function drawSpikeCanvas(ctx,cx,cy,rx,ry,fill,stroke,borderStyle){
    var spikes=14;ctx.beginPath();
    for(var i=0;i<spikes;i++){
      var a=(i/spikes)*Math.PI*2-Math.PI/2;var a2=((i+0.5)/spikes)*Math.PI*2-Math.PI/2;
      if(i===0)ctx.moveTo(cx+Math.cos(a)*rx,cy+Math.sin(a)*ry);
      else ctx.lineTo(cx+Math.cos(a)*rx,cy+Math.sin(a)*ry);
      ctx.lineTo(cx+Math.cos(a2)*rx*1.25,cy+Math.sin(a2)*ry*1.25);
    }
    ctx.closePath();ctx.fillStyle=fill;ctx.fill();
    if(borderStyle!=='none'){ctx.strokeStyle=stroke;ctx.lineWidth=2.5;ctx.lineJoin='round';ctx.stroke();}
  }
  function getTailPolyCanvas(tail,x,y,w,h){
    var cx=x+w/2,cy=y+h*0.46,rx=w*0.47;
    var pts={
  ...
  return{createSVG,drawOnCanvas,TAIL_DIRS,THOUGHT_DOTS_DIR};
})();